...
def progress_bar_callback(current_bytes, total_bytes, message, filename: str):
    """Callback function to show a simple progress bar for downloading.
    ...
    if percent > downloading_progress[filename]["prev_percentage"]:
        ...
        kbps = current_bytes / elapsed_time if elapsed_time else 0
        msg = f"Downloading {filename}... {percent}%\n{kbps/1024:.2f} KB/s"
        if percent == 100:
